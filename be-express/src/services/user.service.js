...
const getUsers = async ({ page = 1, limit = 20, status, search } = {}) => {
...
const updateUserStatus = async (id, status) => {
  ...
  const [updated] = await db.query(
    'SELECT id, name, email, role, status, avatar, created_at FROM users WHERE id = ?', [id]
  );
  return updated[0];
};
...
const getMe = async (userId) => {
  const [rows] = await db.query(
    'SELECT id, name, email, role, status, avatar, created_at FROM users WHERE id = ?', [userId]
  );
  if (rows.length === 0) throw new AppError('User not found', 404);
  return rows[0];
};
